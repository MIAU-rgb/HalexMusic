...
class MusicPlayer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🎵 HalexMusic 2.41V")
        ...
        self.custom_tags = {}
        self.viz_mode = "circular"  # НОВОЕ: "circular" или "bars"
    ...
    # НОВОЕ: ОБНОВЛЁННЫЙ ВИЗУАЛИЗАТОР С ДВУМЯ РЕЖИМАМИ
    ...
    def draw_bar_viz(self, cw, ch):
        """Оригинальный прямоугольный визуализатор"""
        bar_count = 64
    ...
    def draw_circular_viz(self, cw, ch):
        """Круговой визуализатор как в YouTube Music"""
        num_points = 64
        cx, cy = cw // 2, ch // 2
        ...
        # Точки/полоски по окружности
        for i in range(num_points):
            angle = (i / num_points) * 2 * math.pi
            amplitude = self.viz_data[i]
            
            # Радиус точки зависит от амплитуды
            ...
            # Размер точки зависит от амплитуды
            size = 2 + amplitude * 4
            ...
            color = '#{:02x}{:02x}{:02x}'.format(
                min(255, red), min(255, green), min(255, blue)
            )
            ...
            # Соединительная линия от центра
            if amplitude > 0.3:
        ...
        if self.is_playing and not self.is_paused:
            self.viz_canvas.create_text(
                cx, cy,
                text="♪",
                font=('Arial', int(base_radius * 1.2)),
                fill='#ffffff',
                anchor='center'
            )
    
    # НОВОЕ: ПЕРЕКЛЮЧЕНИЕ РЕЖИМА ВИЗУАЛИЗАТОРА
        ...
        if os.path.exists(self.settings_file):
            try:
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    self.viz_mode = data.get('viz_mode', 'circular')  # НОВОЕ
                    self.colors = self.get_color_scheme()
            except:
                pass
    ...
    def edit_tags(self):
        ...
        self.tag_editor_dialog.geometry("350x220")
        ...
        tk.Button(self.tag_editor_dialog, text="💾 Сохранить", font=('Arial', 9, 'bold'),
                 bg='#8A2BE2', fg='white', relief='flat', padx=12, pady=4,
                 command=save_tags).pack(pady=10)
    ...
    def setup_ui(self):
        ...
        tk.Button(top, text="🔄 Виз", **btn_cfg, command=self.toggle_viz_mode).pack(side='left', padx=1)  # НОВОЕ
        ...
        tk.Label(bottom_frame, text="© 2026 HalexMusic 2.41V | github.com/MIAU-rgb/HalexMusic", 
    ...
    def create_mini_player(self):
        ...
        tk.Label(tb, text="HALEX 2.41V", font=('Segoe UI', 5, 'bold'), fg=ra, bg=rs).pack(side='left', padx=4)
    ...
    def refresh_music(self):
        ...
        self.shuffled_tracks = []
        self.original_tracks = []
        messagebox.showinfo("HalexMusic 2.41V", "Музыка обновлена!")
