"""
HalexMusic Player 2.7V
MusicBee-стиль + Classic-стиль + Обложки треков + Update Checker
...
class MusicPlayer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🎵 HalexMusic 2.7V")
        ...
        self.custom_tags = {}
        self.viz_mode = "circular"
        self.ui_style = "musicbee"
        self.current_version = "2.7V"
        self.github_releases_url = "https://github.com/MIAU-rgb/HalexMusic/releases"
        self.github_api_url = "https://api.github.com/repos/MIAU-rgb/HalexMusic/releases/latest"
    ...
    def check_for_updates(self):
        try:
            ...
            req.add_header("User-Agent", "HalexMusic-Update-Checker")
            req.add_header("Accept", "application/vnd.github.v3+json")
            
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode())
                latest_version = data.get("tag_name", "неизвестно")
            ...
            if latest_version != self.current_version:
                messagebox.showinfo(
                    ...
                    f"Дата: {published_at[:10]}\n\n"
                    f"Нажмите OK чтобы перейти на страницу загрузки."
                )
                ...
                messagebox.showinfo(
                    "✅ Обновлений нет",
                    f"Текущая версия: {self.current_version}\n\n"
                    f"У вас последняя версия HalexMusic!\n"
            ...
            if messagebox.askyesno(
                ...
                f"Не удалось подключиться к GitHub.\n\n"
                f"Возможно, нет интернета или GitHub заблокирован.\n\n"
    ...
    def get_color_scheme(self):
        schemes = {
            ...
            "xanth": {
                'bg': '#0a0a0a', 'panel': '#1a1a1a', 'accent': '#9ACD32',
                'text': '#ffffff', 'yellow': '#9ACD32', 'progress': '#9ACD32',
                'btn_bg': '#6B8E23', 'tree_bg': '#1a1a1a', 'selected': '#9ACD32',
                'mini_bg': '#0d1117', 'mini_accent': '#9ACD32', 'mini_text': '#ffffff',
                'mini_secondary': '#161b22', 'hotkeys': '#777', 'copyright': '#666',
                'social_btn': '#6B8E23', 'viz_bg': '#0a0a0a', 'delete_btn': '#8B0000'
            },
            "cyan": {
                'bg': '#0a0a0a', 'panel': '#1a1a1a', 'accent': '#00CED1',
                'text': '#ffffff', 'yellow': '#00CED1', 'progress': '#00CED1',
                'btn_bg': '#008B8B', 'tree_bg': '#1a1a1a', 'selected': '#00CED1',
                'mini_bg': '#0d1117', 'mini_accent': '#00CED1', 'mini_text': '#ffffff',
                'mini_secondary': '#161b22', 'hotkeys': '#777', 'copyright': '#666',
                'social_btn': '#008B8B', 'viz_bg': '#0a0a0a', 'delete_btn': '#8B0000'
            },
            "eryth": {
                'bg': '#0a0a0a', 'panel': '#1a1a1a', 'accent': '#FF1493',
                'text': '#ffffff', 'yellow': '#FF1493', 'progress': '#FF1493',
                'btn_bg': '#C71585', 'tree_bg': '#1a1a1a', 'selected': '#FF1493',
                'mini_bg': '#0d1117', 'mini_accent': '#FF1493', 'mini_text': '#ffffff',
                'mini_secondary': '#161b22', 'hotkeys': '#777', 'copyright': '#666',
                'social_btn': '#C71585', 'viz_bg': '#0a0a0a', 'delete_btn': '#8B0000'
            },
            "chlor": {
                'bg': '#0a0a0a', 'panel': '#1a1a1a', 'accent': '#32CD32',
                'text': '#ffffff', 'yellow': '#32CD32', 'progress': '#32CD32',
                'btn_bg': '#228B22', 'tree_bg': '#1a1a1a', 'selected': '#32CD32',
                'mini_bg': '#0d1117', 'mini_accent': '#32CD32', 'mini_text': '#ffffff',
                'mini_secondary': '#161b22', 'hotkeys': '#777', 'copyright': '#666',
                'social_btn': '#228B22', 'viz_bg': '#0a0a0a', 'delete_btn': '#8B0000'
            }
        }
        ...
        if os.path.exists(self.settings_file):
            try:
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    self.viz_mode = data.get('viz_mode', 'circular')
                    self.ui_style = data.get('ui_style', 'musicbee')
                    self.colors = self.get_color_scheme()
            except:
                pass
            ...
            if os.path.isdir(folder_path):
                ...
                for file in os.listdir(folder_path):
                    if file.endswith('.mp3'):
                        ...
                        track_info = self.get_track_info(track_path)
                        if track_path in self.custom_tags:
                            track_info['title'] = self.custom_tags[track_path]['title']
                            track_info['artist'] = self.custom_tags[track_path]['artist']
    ...
    def get_album_art(self, filepath, size=(60, 60)):
        try:
            audio = ID3(filepath)
    ...
    # ========== NEW INTERFACE ==========
    def setup_ui_musicbee(self):
        c = self.colors

        top = tk.Frame(self.root, bg=c['bg'], height=36)
        top.pack(fill='x', padx=5, pady=(5, 0))
        top.pack_propagate(False)

        btn_cfg = {'font': ('Arial', 8), 'bg': c['btn_bg'], 'fg': 'white', 'relief': 'flat', 'padx': 8, 'pady': 3}

        tk.Button(top, text="🔍", **btn_cfg, command=self.show_search).pack(side='left', padx=1)
        tk.Button(top, text="🖥", **btn_cfg, command=self.toggle_mini_player).pack(side='left', padx=1)
        tk.Button(top, text="✏️ Теги", **btn_cfg, command=self.edit_tags).pack(side='left', padx=1)
        tk.Button(top, text="🔄 Виз", **btn_cfg, command=self.toggle_viz_mode).pack(side='left', padx=1)
        tk.Button(top, text="🎨 Old interface", **btn_cfg, command=self.toggle_ui_style).pack(side='left', padx=1)

        modes = [
            ("👁", "normal"), ("🟢", "deuteranopia"), ("🔵", "tritanopia"),
            ("🔴", "protanopia"), ("🟡", "xanth"), ("🩵", "cyan"),
            ("❤️", "eryth"), ("💚", "chlor")
        ]
        ...
        self.current_track_label = tk.Label(
            top, text="",
            font=('Arial', 10, 'bold'),
            fg=c['yellow'], bg=c['bg'], anchor='e'
        )
        self.current_track_label.pack(side='right', padx=10, fill='x', expand=True)

        main = tk.Frame(self.root, bg=c['bg'])
        main.pack(fill='both', expand=True, padx=8, pady=5)

        left = tk.Frame(main, bg=c['panel'], width=160)
        left.pack(side='left', fill='y', padx=(0, 6))
        left.pack_propagate(False)

        tk.Label(left, text="📂 Плейлисты", font=('Arial', 10, 'bold'), fg='white', bg=c['panel']).pack(pady=8)

        self.playlist_listbox = tk.Listbox(
            left, bg=c['bg'], fg='white',
            selectbackground=c['selected'],
            font=('Arial', 9), relief='flat', bd=0, height=12
        )
        self.playlist_listbox.pack(fill='both', expand=True, padx=6, pady=4)
        self.playlist_listbox.bind('<<ListboxSelect>>', self.on_playlist_select)

        pl_btns = tk.Frame(left, bg=c['panel'])
        pl_btns.pack(pady=4)

        tk.Button(pl_btns, text="➕", font=('Arial', 8), bg=c['btn_bg'], fg='white',
                  relief='flat', padx=8, command=self.create_playlist).pack(side='left', padx=1)
        tk.Button(pl_btns, text="🔄", font=('Arial', 8), bg=c['btn_bg'], fg='white',
                  relief='flat', padx=8, command=self.refresh_music).pack(side='left', padx=1)
        tk.Button(pl_btns, text="✏️", font=('Arial', 8), bg='#333', fg='white',
                  relief='flat', padx=8, command=self.rename_playlist).pack(side='left', padx=1)
        tk.Button(pl_btns, text="💾", font=('Arial', 8), bg=c['btn_bg'], fg='white',
                  relief='flat', padx=8, command=self.export_playlist).pack(side='left', padx=1)
        tk.Button(pl_btns, text="🗑️", font=('Arial', 8), bg=c['delete_btn'], fg='white',
                  relief='flat', padx=8, command=self.delete_playlist).pack(side='left', padx=1)

        right = tk.Frame(main, bg=c['bg'])
        right.pack(side='right', fill='both', expand=True)

        info_frame = tk.Frame(right, bg=c['panel'], height=80)
        info_frame.pack(fill='x', pady=(0, 8))
        info_frame.pack_propagate(False)

        cover_frame = tk.Frame(info_frame, bg=c['accent'], width=60, height=60)
        cover_frame.pack(side='left', padx=10, pady=10)
        cover_frame.pack_propagate(False)
        self.mb_cover_label = tk.Label(cover_frame, text="🎵", font=('Arial', 24), bg=c['accent'], fg='white')
        self.mb_cover_label.pack(expand=True)

        track_info = tk.Frame(info_frame, bg=c['panel'])
        track_info.pack(side='left', fill='both', expand=True, pady=10)

        self.mb_track_title = tk.Label(track_info, text="Нет трека",
            font=('Arial', 14, 'bold'), fg='white', bg=c['panel'], anchor='w')
        self.mb_track_title.pack(fill='x')

        self.mb_track_artist = tk.Label(track_info, text="",
            font=('Arial', 10), fg='#aaa', bg=c['panel'], anchor='w')
        self.mb_track_artist.pack(fill='x')

        prog_frame = tk.Frame(right, bg=c['bg'])
        prog_frame.pack(fill='x', pady=3)

        self.current_time_label = tk.Label(prog_frame, text="0:00",
            font=('Arial', 8), fg='#aaa', bg=c['bg'])
        ...
        self.total_time_label = tk.Label(prog_frame, text="0:00",
            font=('Arial', 8), fg='#aaa', bg=c['bg'])
        self.total_time_label.pack(side='right')

        ctrl_frame = tk.Frame(right, bg=c['bg'])
        ...
        for text, cmd in control_buttons:
            tk.Button(ctrl_frame, text=text, font=('Arial', 16), bg=c['panel'], fg='white',
                     relief='flat', width=2, command=cmd).pack(side='left', padx=3)

        self.repeat_btn = tk.Button(ctrl_frame, text="➡️", font=('Arial', 12), bg=c['panel'], fg='white',
                                 relief='flat', width=2, command=self.toggle_repeat)
        self.repeat_btn.pack(side='left', padx=3)
        self.update_repeat_button()

        self.volume_var = tk.DoubleVar(value=self.volume)
        self.volume_scale = ttk.Scale(ctrl_frame, from_=0, to=100, variable=self.volume_var,
                                       orient='horizontal', command=self.change_volume, length=120)
        self.volume_scale.pack(side='right', padx=6)

        self.volume_label = tk.Label(ctrl_frame, text=f"{self.volume}%",
            font=('Arial', 9), fg='white', bg=c['bg'], width=4)
        self.volume_label.pack(side='right')
        tk.Label(ctrl_frame, text="🔊", font=('Arial', 10), fg='white', bg=c['bg']).pack(side='right')

        track_header = tk.Frame(right, bg=c['bg'])
        track_header.pack(fill='x', pady=(8, 2))
        tk.Label(track_header, text="🎶 Треки", font=('Arial', 11, 'bold'),
                fg='white', bg=c['bg']).pack(side='left')
        if self.shuffle_mode:
            tk.Label(track_header, text="(Shuffle)", font=('Arial', 8),
                    fg=c['accent'], bg=c['bg']).pack(side='left', padx=6)
        ...
        self.tracks_tree.column('#', width=30)
        self.tracks_tree.column('Название', width=240)
        self.tracks_tree.column('Исполнитель', width=160)
        self.tracks_tree.column('Время', width=50)

        style = ttk.Style()
        style.theme_use("default")
        style.configure("Treeview", background=c['tree_bg'], foreground="white",
                       fieldbackground=c['tree_bg'], rowheight=24, font=('Arial', 9))
        ...
        viz_frame = tk.Frame(right, bg=c['viz_bg'], height=60)
        viz_frame.pack(fill='x', pady=(4, 0))
        ...
        tk.Label(bottom_frame, text=f"© 2026 HalexMusic {self.current_version} | github.com/MIAU-rgb/HalexMusic",
                font=('Arial', 6), fg=c['copyright'], bg=c['bg']).pack(side='left', padx=5)

        update_btn_cfg = {'font': ('Arial', 7, 'bold'), 'bg': c['accent'], 'fg': 'white',
                          'relief': 'flat', 'padx': 8, 'pady': 2}
        tk.Button(bottom_frame, text="🔄 Update", **update_btn_cfg,
                 command=self.check_for_updates).pack(side='right', padx=5)

        social_frame = tk.Frame(bottom_frame, bg=c['bg'])
        social_frame.pack(side='right', padx=5)

        social_btn_cfg = {
            'font': ('Arial', 7, 'bold'), 'bg': c['social_btn'], 'fg': 'white',
    ...
    def setup_ui(self):
        ...
        tk.Button(top, text="✏️ Теги", **btn_cfg, command=self.edit_tags).pack(side='left', padx=1)
        tk.Button(top, text="🔄 Виз", **btn_cfg, command=self.toggle_viz_mode).pack(side='left', padx=1)
        tk.Button(top, text="🎨 New interface", **btn_cfg, command=self.toggle_ui_style).pack(side='left', padx=1)
        ...
        self.repeat_btn = tk.Button(ctrl_vol, text="➡️", font=('Arial', 10), bg=c['panel'], fg='white',
                                 relief='flat', width=2, command=self.toggle_repeat)
        ...
        social_btn_cfg = {
            'font': ('Arial', 6, 'bold'), 'bg': c['social_btn'], 'fg': 'white',
        ...
        if self.current_track_index >= 0 and self.current_playlist:
            ...
            if self.current_track_index < len(tracks):
                ...
                if hasattr(self, 'current_track_label'):
                    self.current_track_label.config(text=f"🎵 {track['title'][:50]}")
                if hasattr(self, 'mb_track_title'):
                    self.mb_track_title.config(text=track['title'][:40])
                    self.mb_track_artist.config(text=track['artist'][:40])
                if hasattr(self, 'mb_cover_label'):
                    album_art = self.get_album_art(track['path'])
                    if album_art:
                        self.mb_cover_label.config(image=album_art, text="")
                        self.mb_cover_label.image = album_art
                    else:
                        self.mb_cover_label.config(image="", text="🎵")
    ...
    def draw_viz_bars(self):
        ...
        if cw < 10 or ch < 10:
            return
        if self.viz_mode == "circular":
            self.draw_circular_viz(cw, ch)
        else:
            self.draw_bar_viz(cw, ch)

    def draw_bar_viz(self, cw, ch):
        bar_count = 64
    ...
    def draw_circular_viz(self, cw, ch):
        num_points = 64
        cx, cy = cw // 2, ch // 2
        base_radius = min(cw, ch) * 0.12
        max_radius = min(cw, ch) * 0.42

        self.viz_canvas.create_oval(
            cx - base_radius * 0.8, cy - base_radius * 0.8,
            cx + base_radius * 0.8, cy + base_radius * 0.8,
        ...
        for i in range(num_points):
            ...
            amplitude = self.viz_data[i]
            r = base_radius + amplitude * (max_radius - base_radius)
            x = cx + r * math.cos(angle)
            y = cy + r * math.sin(angle)
            size = 2 + amplitude * 4
            hue = (i * 5.6 + int(self.current_time * 3)) % 360
            red = int((math.sin(math.radians(hue)) * 127 + 128))
            green = int((math.sin(math.radians(hue + 120)) * 127 + 128))
            blue = int((math.sin(math.radians(hue + 240)) * 127 + 128))
            brightness = 0.3 + amplitude * 0.7
            red = int(red * brightness)
            green = int(green * brightness)
            blue = int(blue * brightness)
            color = '#{:02x}{:02x}{:02x}'.format(min(255, red), min(255, green), min(255, blue))
            self.viz_canvas.create_oval(x - size, y - size, x + size, y + size, fill=color, outline='', width=0)
            if amplitude > 0.3:
                line_x = cx + base_radius * math.cos(angle)
                line_y = cy + base_radius * math.sin(angle)
                self.viz_canvas.create_line(line_x, line_y, x, y, fill=color, width=1)

        if self.is_playing and not self.is_paused:
            self.viz_canvas.create_text(cx, cy, text="♪", font=('Arial', int(base_radius * 1.2)),
    ...
    # ========== РЕДАКТОР ТЕГОВ ==========
    def edit_tags(self):
        if self.current_playlist is None or self.current_track_index < 0:
            messagebox.showwarning("Теги", "Сначала выберите трек!")
            return
        tracks = self.playlists[self.current_playlist]['tracks']
        if self.current_track_index >= len(tracks):
            return
        track = tracks[self.current_track_index]
        self.tag_editor_dialog = tk.Toplevel(self.root)
        self.tag_editor_dialog.title("Редактор тегов")
        self.tag_editor_dialog.geometry("380x280")
        self.tag_editor_dialog.configure(bg='#1a1a1a')
        tk.Label(self.tag_editor_dialog, text="Редактировать отображение", font=('Arial', 11, 'bold'),
                fg='white', bg='#1a1a1a').pack(pady=8)
        tk.Label(self.tag_editor_dialog, text="Файл:", fg='#aaa', bg='#1a1a1a').pack()
        tk.Label(self.tag_editor_dialog, text=track['filename'][:40], fg='white', bg='#1a1a1a').pack()
        tk.Label(self.tag_editor_dialog, text="(меняется только в плеере, не в файле)",
                font=('Arial', 6), fg='#666', bg='#1a1a1a').pack()
        tk.Label(self.tag_editor_dialog, text="Название:", fg='#aaa', bg='#1a1a1a').pack(pady=(8, 0))
        title_entry = tk.Entry(self.tag_editor_dialog, font=('Arial', 9), bg='#0a0a0a', fg='white')
        title_entry.insert(0, track['title'])
        title_entry.pack(pady=2, ipady=2, ipadx=6)
        tk.Label(self.tag_editor_dialog, text="Исполнитель:", fg='#aaa', bg='#1a1a1a').pack()
        artist_entry = tk.Entry(self.tag_editor_dialog, font=('Arial', 9), bg='#0a0a0a', fg='white')
        artist_entry.insert(0, track['artist'])
        artist_entry.pack(pady=2, ipady=2, ipadx=6)

        def save_tags():
            new_title = title_entry.get().strip()
            new_artist = artist_entry.get().strip()
            if not new_title:
                messagebox.showwarning("Теги", "Название не может быть пустым!")
                return
            track_path = track['path']
            self.custom_tags[track_path] = {'title': new_title, 'artist': new_artist}
            track['title'] = new_title
            track['artist'] = new_artist
            self.update_tracks_list()
            if self.current_track_index >= 0 and self.current_track_index < len(tracks):
                if hasattr(self, 'current_track_label'):
                    self.current_track_label.config(text=f"🎵 {new_title[:50]}")
                if hasattr(self, 'mb_track_title'):
                    self.mb_track_title.config(text=new_title[:40])
                    self.mb_track_artist.config(text=new_artist[:40])
            self.save_info()
            self.save_settings()
            self.tag_editor_dialog.destroy()
            messagebox.showinfo("Теги", "Изменения сохранены!")

        tk.Button(self.tag_editor_dialog, text="💾 Сохранить", font=('Arial', 10, 'bold'),
                 bg='#8A2BE2', fg='white', relief='flat', padx=15, pady=6,
                 command=save_tags).pack(pady=15)
    ...
    def delete_playlist(self):
        ...
        result = messagebox.askyesno("Удаление плейлиста",
            f"Вы уверены что хотите удалить плейлист '{playlist_name}'?\n\nЭто удалит папку с музыкой безвозвратно!")
        if result:
            ...
            if self.is_playing and self.current_playlist not in self.playlists:
                ...
                if hasattr(self, 'mb_track_title'):
                    self.mb_track_title.config(text="Нет трека")
                    self.mb_track_artist.config(text="")
    ...
    def export_playlist(self):
        ...
        if filepath:
            playlist_data = {'name': self.playlists[self.current_playlist]['name'],
                'exported': datetime.now().strftime("%d.%m.%Y %H:%M:%S"), 'tracks': []}
            for track in tracks:
                playlist_data['tracks'].append({'title': track['title'], 'artist': track['artist'],
                    'duration': track['duration'], 'filepath': track['path']})
    ...
    def create_mini_player(self):
        ...
        tk.Label(tb, text="HALEX 2.7V", font=('Segoe UI', 5, 'bold'), fg=ra, bg=rs).pack(side='left', padx=4)
        ...
        bottom_panel = tk.Frame(ct, bg=rb)
        bottom_panel.pack(fill='x', pady=1)
        
        vf = tk.Frame(bottom_panel, bg=rb)
        vf.pack(side='left')
        ...
        if self.ui_style == "musicbee":
            cover_frame = tk.Frame(bottom_panel, bg=rs, width=38, height=38)
            cover_frame.pack(side='right', padx=(5, 0))
            cover_frame.pack_propagate(False)
            self.mini_cover_label = tk.Label(cover_frame, text="🎵", font=('Arial', 14), bg=rs, fg=ra)
            self.mini_cover_label.pack(expand=True)
        else:
            self.mini_cover_label = None
    ...
    def do_move_mini(self, event):
        if self.mini_x and self.mini_y:
            ...
            screen_w = self.root.winfo_screenwidth()
            screen_h = self.root.winfo_screenheight()
            mini_w, mini_h = 280, 130
            min_x, min_y = 0, 0
            max_x = screen_w - mini_w - 10
            max_y = screen_h - mini_h - 48 - 10
        ...
        if self.mini_player and self.mini_player.winfo_exists():
            ...
            if self.current_playlist and self.current_track_index >= 0:
                ...
                if self.current_track_index < len(tracks):
                    ...
                    if self.ui_style == "musicbee" and hasattr(self, 'mini_cover_label') and self.mini_cover_label:
                        album_art = self.get_album_art(track['path'], size=(34, 34))
                        if album_art:
                            self.mini_cover_label.config(image=album_art, text="")
                            self.mini_cover_label.image = album_art
                        else:
                            self.mini_cover_label.config(image="", text="🎵")
    ...
    def play_current_track(self):
        if self.current_playlist and self.current_track_index >= 0:
            ...
            if self.current_track_index < len(tracks):
                ...
                if hasattr(self, 'mb_track_title'):
                    self.mb_track_title.config(text=track['title'][:40])
                if hasattr(self, 'mb_track_artist'):
                    self.mb_track_artist.config(text=track['artist'][:40])
        ...
        if self.current_playlist:
            ...
            if self.current_track_index < len(tracks) - 1:
                ...
                if self.repeat_mode == "all":
                    ...
                    pygame.mixer.music.stop()
                    if hasattr(self, 'current_track_label'):
                        self.current_track_label.config(text="🎵 Конец плейлиста")
                    if hasattr(self, 'mb_track_title'):
                        self.mb_track_title.config(text="Конец плейлиста")
                        self.mb_track_artist.config(text="")
                    self.update_tracks_list()
                    return
            self.play_current_track()
    ...
    def update_tracks_list(self):
        ...
        if self.current_playlist and self.current_playlist in self.playlists:
            ...
            for i, track in enumerate(tracks, 1):
                self.tracks_tree.insert('', 'end', values=(i, f"♫ {track['title'][:26]}", track['artist'][:22], track['duration_str']))
    ...
    def refresh_music(self):
        ...
        self.shuffled_tracks = []
        self.original_tracks = []
        messagebox.showinfo(f"HalexMusic {self.current_version}", "Музыка обновлена!")
