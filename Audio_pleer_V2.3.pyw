...
class MusicPlayer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🎵 HalexMusic 2.3V")
        self.root.geometry("750x520")
        self.root.configure(bg='#0a0a0a')
        self.root.minsize(650, 470)
        
        # ВАЖНО: Большой буфер для лучшего захвата звука
        ...
        # Визуализатор - 64 полоски для более детального отображения
        self.viz_data = [0.0] * 64
        self.viz_peaks = [0.0] * 64  # Пики для эффекта затухания
    ...
    def update_visualizer(self):
        ...
        if hasattr(self, 'viz_canvas') and self.viz_canvas:
            if self.is_playing and not self.is_paused:
                    ...
                    if sound_buffer is not None and len(sound_buffer) > 0:
                        ...
                        for i in range(num_bars):
                            ...
                            if start < len(sound_buffer):
                                ...
                                if len(chunk) > 0:
                                    # Считаем RMS (среднеквадратичную амплитуду) - более чувствительно!
                            ...
                            # Басы (первые 16 полосок) - ОЧЕНЬ чувствительные
                            if i < 16:
                                amplitude *= 3.0  # БАСЫ УСИЛЕНЫ В 3 РАЗА
                            # Средние частоты (16-48)
                            elif i < 48:
                                amplitude *= 2.0  # УСИЛЕНЫ В 2 РАЗА
                            # Верха (48-64)
                            else:
                                amplitude *= 1.5  # УСИЛЕНЫ В 1.5 РАЗА
                            
                            # Применяем громкость плеера
                            amplitude *= (0.5 + self.volume / 100.0)
                            
                            # Ограничиваем максимум
                            amplitude = min(1.0, amplitude)
                            
                            # Плавное нарастание (быстрое) и затухание (медленное)
                            ...
                            if amplitude > self.viz_peaks[i]:
                                ...
                                self.viz_peaks[i] *= 0.95  # Медленное падение пиков
                    else:
                        # Плавное затухание если буфер пуст
                        for i in range(64):
                            self.viz_data[i] *= 0.8
                            self.viz_peaks[i] *= 0.9
                except Exception as e:
                    # Fallback: симулируем на основе громкости
                ...
                # Музыка на паузе - плавное затухание
                for i in range(64):
    ...
    def setup_ui(self):
        ...
        tk.Label(bottom_frame, text="© 2026 HalexMusic 2.3V | github.com/MIAU-rgb/AudioPleer", 
    ...
    def create_mini_player(self):
        ...
        tk.Label(tb, text="HALEX 2.3V", font=('Segoe UI', 5, 'bold'), fg=ra, bg=rs).pack(side='left', padx=4)
    ...
    def refresh_music(self):
        ...
        self.shuffled_tracks = []
        self.original_tracks = []
        messagebox.showinfo("HalexMusic 2.3V", "Музыка обновлена!")
