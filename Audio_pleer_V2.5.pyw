"""
HalexMusic Player Prototype 2.5V (Fixed & Improved)
MusicBee-стиль интерфейса (ПРОТОТИП) с обложками треков
...
class MusicPlayer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🎵 HalexMusic 2.5V Prototype")
    ...
    def setup_ui_musicbee(self):
        ...
        bottom_frame = tk.Frame(right, bg=c['bg'], height=18)
        bottom_frame.pack(fill='x', side='bottom', pady=(2, 0))
        bottom_frame.pack_propagate(False)

        tk.Label(bottom_frame, text="© 2026 HalexMusic 2.5V Prototype | github.com/MIAU-rgb/HalexMusic",
    ...
    # ========== CLASSIC INTERFACE (OLD INTERFACE) ==========
    def setup_ui(self):
        c = self.colors
        ...
        tk.Label(bottom_frame, text="© 2026 HalexMusic 2.5V | github.com/MIAU-rgb/HalexMusic",
                font=('Arial', 6), fg=c['copyright'], bg=c['bg']).pack(side='left', padx=5)
    ...
    def create_mini_player(self):
        ...
        tk.Label(tb, text="HALEX 2.5V", font=('Segoe UI', 5, 'bold'), fg=ra, bg=rs).pack(side='left', padx=4)
    ...
    def play_current_track(self):
        if self.current_playlist and self.current_track_index >= 0:
            ...
            if self.current_track_index < len(tracks):
                ...
                # Обновляем старый интерфейс
                if hasattr(self, 'current_track_label'):
                    self.current_track_label.config(text=f"🎵 {track['title'][:50]}")

                # Обновляем MusicBee-интерфейс
                if hasattr(self, 'mb_track_title'):
                    self.mb_track_title.config(text=track['title'][:40])
                if hasattr(self, 'mb_track_artist'):
                    self.mb_track_artist.config(text=track['artist'][:40])

                # Обновляем обложку в MusicBee-интерфейсе
                if hasattr(self, 'mb_cover_label'):
                    album_art = self.get_album_art(track['path'])
                    if album_art:
                        self.mb_cover_label.config(image=album_art, text="")
    ...
    def refresh_music(self):
        ...
        self.shuffled_tracks = []
        self.original_tracks = []
        messagebox.showinfo("HalexMusic 2.5V", "Музыка обновлена!")
